"""Persistence: what you've been shown, what you thought of it, and the interest
profile learned from those two things.

The store is deliberately boring. All the learning logic lives in `learn.py`;
this module only reads and writes.

It talks to a driver rather than to SQLite directly, so the same code runs
against a local file or against a remote libSQL database — which is what makes
it work on a serverless host with no disk. See `drivers.py`.
"""
...
from .drivers import Driver, open_driver
...
class Store:
    def __init__(self, path: str | Path, auth_token: str | None = None) -> None:
        self.driver: Driver = open_driver(path, auth_token)
        self.driver.script(SCHEMA)
    ...
    def known_ids(self, ids: list[str]) -> set[str]:
        ...
        for start in range(0, len(ids), 500):
            ...
            rows = self.driver.query(f"SELECT id FROM items WHERE id IN ({placeholders})", chunk)
            found.update(row["id"] for row in rows)
        return found

    def get_item(self, item_id: str) -> Item | None:
        rows = self.driver.query("SELECT * FROM items WHERE id = ?", (item_id,))
    ...
    def prune(self, older_than_days: int = 45) -> int:
        ...
        # Remote libSQL has no ON DELETE CASCADE guarantee without the pragma,
        # so dependent rows go first and explicitly.
        for table in ("verdicts", "shown"):
            self.driver.write(
                f"DELETE FROM {table} WHERE item_id IN "
                "(SELECT id FROM items WHERE first_seen_at < ?)",
                (cutoff,),
            )
        return self.driver.write("DELETE FROM items WHERE first_seen_at < ?", (cutoff,))
    ...
    def last_digest_at(self) -> datetime | None:
        rows = self.driver.query("SELECT MAX(digest_at) AS latest FROM shown")
        latest = rows[0]["latest"] if rows else None
        return _parse_dt(latest) if latest else None
    ...
    def feedback_counts(self) -> dict[str, int]:
        rows = self.driver.query("SELECT signal, COUNT(*) AS n FROM feedback GROUP BY signal")
        return {row["signal"]: row["n"] for row in rows}

    # -- profile ---------------------------------------------------------

    def profile(self, kind: str) -> dict[str, float]:
        rows = self.driver.query("SELECT key, weight FROM profile WHERE kind = ?", (kind,))
    ...
    def bump_profile(self, kind: str, deltas: dict[str, float], decay: float = 1.0) -> None:
        ...
        statements.append(
            ("DELETE FROM profile WHERE kind = ? AND ABS(weight) < 0.02 AND count < 3", (kind,))
        )
        self.driver.batch(statements)
